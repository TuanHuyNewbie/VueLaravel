            ...
            <div class="elevation-4" style="height: 60px;">
                <v-row style="width: 100%;">
                    ...
                    <v-col cols="6" style="width: 100%; display: flex; justify-content: space-evenly;">
                        <v-btn :disabled="disAble" style="background-color: green;" @click="addMucDichSuDungDat()"><b>+ Thêm
                                Mới</b></v-btn>
                ...
                <v-data-table :headers="header" :items="desserts" :items-per-page="5" :search="search" class="elevation-5">
                    ...
                    <template v-slot:item.action="{ item }">
                        ...
                        <div v-show="item.id_muc_dich_su_dung_dat == idAction && action">
                            <v-btn color="green"
                                @click="reviewMuctiMucDichSuDungDat(item.id_muc_dich_su_dung_dat)"><v-icon>fa
                                    fa-eye</v-icon></v-btn>
                            <v-btn :disabled="disAble" color="primary"
                                @click="updateMucDichSuDungDat(item.id_muc_dich_su_dung_dat)"><v-icon>fa
                                    fa-pencil</v-icon></v-btn>
                            <v-btn :disabled="disAble" color="error"
                                @click="deleteMucDichSuDungDat(item.id_muc_dich_su_dung_dat)"><v-icon>fa
    ...
    methods: {
        ...
        exportToExCel() {
            ...
            this.desserts.forEach((dessert) => {
                let obj = {
                    "ID Mục Đích Sử Dụng Đất": dessert.id_muc_dich_su_dung_dat,
                    "Ký Hiệu": dessert.ky_hieu,
                    "Hiện Trạng": dessert.hien_trang,
