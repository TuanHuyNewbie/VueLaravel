            ...
            <v-card elavation="7" style="width: 100%; height: 100%; display: flex; justify-content: center; align-items: center;">
                <v-form @submit.prevent="addNongTruong()" v-model="valid">
                    <div style="display: flex;">
                        <v-text-field label="ID Phân Loại (*)" :rules="rules" hide-details="auto" outlined v-model="id_phan_loai"></v-text-field>
                        <v-select item-text="ky_hieu" item-value="id_hien_trang_vuon_cay" :items="htvc" label="Kí Hiệu Hiện Trạng Vườn Cây (*)" :rules="rules" hide-details="auto" outlined v-model="id_hien_trang_vuon_cay"></v-select>
                        <v-select item-text="ky_hieu" item-value="id_muc_dich_su_dung_da" :items="mdsdd" label="Kí Hiệu Mục Đích Sử Dụng Đất (*)" :rules="rules" hide-details="auto" outlined v-model="id_muc_dich_su_dung_dat"></v-select>
                        <v-text-field label="Kí Hiệu Phân Loại(*)" :rules="rules" hide-details="auto" outlined v-model="ma_nong_truong"></v-text-field>
                        <v-text-field label="Diễn Giải" :rules="rules" hide-details="auto" outlined v-model="hidden"></v-text-field>
                    </div>
...
export default {
    ...
    methods: {
        ...
        addNongTruong(){
            ...
            formData.append('ky_hieu_phan_loai', this.ky_hieu_phan_loai);
            formData.append('dien_giai', this.dien_giai);
            this.axios.post('/api/create-phan-loai', formData)
                .then((response) => {
                    this.home()
                })
                .catch((error) => {
